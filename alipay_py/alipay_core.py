...
def make_sign(message):
    ...
    sign = rsa.sign(message.encode('utf-8'), private_key, SIGN_TYPE)
...
def check_sign(message, sign):
    ...
    return rsa.verify(message.encode('utf-8'), sign, pubkey)
...
def check_ali_sign(message, sign):
    ...
    res = False
    try:
        res = rsa.verify(message.encode('utf-8'), sign, pubkey)
...
def make_payment_request(params_dict):
    ...
    sign = make_sign(query_str).decode('utf-8') #生成签名
    #sign = urllib.parse.quote_plus(sign)
    #print (sign)
    #c = check_sign(message=query_str,sign=sign)
    #print (c)
